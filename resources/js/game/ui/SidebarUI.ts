...
export class SidebarUI {
  ...
  setCallbacks(onVisit: VisitUserCallback, saveCb: () => void, onPlaceItem?: PlaceItemCallback): void {
          ...
          (item) =>
            `<div class="flex justify-between items-center py-1 border-b dark:border-gray-700 last:border-0">
              ...
              <div class="flex items-center gap-2">
                ...
                <button class="place-item-btn text-xs px-2 py-0.5 bg-indigo-600 text-white rounded hover:bg-indigo-700" data-item-id="${item.item_id}">Place</button>
              </div>
            </div>`
        )
        .join('');
